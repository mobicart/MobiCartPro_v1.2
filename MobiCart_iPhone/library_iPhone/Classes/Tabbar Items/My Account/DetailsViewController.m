...
- (void)loadView {
	...
	if ([arrInfoAccount count] == 0) {
		...
		[btnEdit setBackgroundColor:navBarColorPreference];
...
- (void)editInfo {
	...
	if (![txtDetails[2] isEnabled]) {
		...
		if (([txtDetails[0].text length] == 0) || ([txtDetails[1].text length] == 0) || ([txtDetails[2].text length] == 0) || ([txtDetails[3].text length] == 0) || ([txtDetails[4].text length] == 0) || ([txtDetails[5].text length] == 0) || ([txtDeliveryDetails[0].text length] == 0) || [txtDeliveryDetails[1].text length] == 0 || [txtDeliveryDetails[2].text length] == 0 || [txtDeliveryDetails[3].text length] == 0 || [txtDeliveryDetails[4].text length] == 0) {
			...
			// Tuyen note:
			// txtDetails[0]: eMailAddress
			// txtDetails[1]: streetAddress
			// txtDetails[2]: city
			// txtDetails[4]: state
			// txtDetails[3]: country
			// txtDetails[5]: pincode
			// txtDeliveryDetails[0]: deliveryStreetAddress
			// txtDeliveryDetails[1]: deliveryCity
			// txtDeliveryDetails[2]: deliveryCountry
			// txtDeliveryDetails[3]: deliveryState
			// txtDeliveryDetails[4]: deliveryPincode
			...
			for (int i = 0; i < [arrTemp count]; i++) {
				// 01/12/2014 Tuyen close fix bug 27849 - Crash when edit account
...
//				}
				...
				// 01/12/2014 Tuyen new code fix bug 27849 - Crash when edit account
			...
			// 01/12/2014 Tuyen close fix bug 27704 - Shipping fee is not correct when user changes their profile
			NSString *strState;
			for (int i = 0; i < [arrTemp count]; i++) {
				if ([txtDeliveryDetails[3].text isEqualToString:[[arrTemp objectAtIndex:i]valueForKey:@"sState"]]) {
					strState = [[arrTemp objectAtIndex:i] valueForKey:@"stateId"];
					currentStateIdForDelivery = [strState integerValue];
				}
			}
			for (int i = 0; i < [arrTempShippingCountries count]; i++) {
				if ([txtDeliveryDetails[3].text isEqualToString:[[arrTempShippingCountries objectAtIndex:i]valueForKey:@"sState"]]) {
					strState = [[arrTempShippingCountries objectAtIndex:i] valueForKey:@"stateId"];
					currentStateIdForDelivery = [strState integerValue];
				}
			}
			// End
...
- (void)displayViewForRegistration {
	...
	[btnSubmit setBackgroundColor:navBarColorPreference];
...
- (void)displayViewForLogin {
	...
	[btnLogin setBackgroundColor:navBarColorPreference];
	...
	[btnCreateLogin setBackgroundColor:navBarColorPreference];
