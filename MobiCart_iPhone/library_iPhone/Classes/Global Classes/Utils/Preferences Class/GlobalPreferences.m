...
#define colorDict dictFeatures //[dictFeatures objectForKey:@"colorscheme"]
	if (![colorDict isEqual:[NSNull null]]) {
		if ([colorDict objectForKey:@"sBgColor"]) {
			...
			navBarColorPreference = _savedPreferences.bgColor;
		}
...
+ (void)setSettingsOfUserAndOtherDetails:(NSDictionary *)_dictSettings {
	...
	//************************************2c2p Mode***************
	NSString *_2c2pModeEnable = [dictUserSettings valueForKey:@"p2c2pUsed"];
	if (![_2c2pModeEnable isKindOfClass:[NSNull class]])
		[GlobalPreferences set2c2pModeEnable:_2c2pModeEnable];

	NSString *_2c2pMerchantId = [dictUserSettings valueForKey:@"p2c2pMerchantId"];
	if (![_2c2pMerchantId isKindOfClass:[NSNull class]])
		[GlobalPreferences set2c2pMerchantId:_2c2pMerchantId];

	NSString *_2c2pSecrectKey = [dictUserSettings valueForKey:@"p2c2pSecrectKey"];
	if (![_2c2pSecrectKey isKindOfClass:[NSNull class]]) {
		[GlobalPreferences set2c2pSecrectKey:_2c2pSecrectKey];
	}

    // 13/01/2015 Tuyen close code fix bug crash when pay with 2c2p
//	NSString *_2c2pCurrencyNumber = [dictUserSettings valueForKey:@"p2c2pcurrencyNumber"];
    //
    ...
    NSString *_2c2pCurrencyNumber = [[dictUserSettings valueForKey:@"p2c2pcurrencyNumber"] stringValue];
    //
	if (![_2c2pCurrencyNumber isKindOfClass:[NSNull class]]) {
		[GlobalPreferences set2c2pCurrencyNumber:_2c2pCurrencyNumber];
	}

	//************************************iPay88 Mode***************
	NSString *iPay88ModeEnable = [dictUserSettings valueForKey:@"ipay88Used"];
	if (![iPay88ModeEnable isKindOfClass:[NSNull class]])
		[GlobalPreferences setiPay88ModeEnable:iPay88ModeEnable];

	NSString *iPay88MerchatKey = [dictUserSettings valueForKey:@"ipay88MerchantKey"];
	if (![iPay88LiveToken isKindOfClass:[NSNull class]]) {
		[GlobalPreferences setiPay88MerchantKey:iPay88MerchatKey];
	}

	NSString *iPay88MerchantCode = [dictUserSettings valueForKey:@"ipay88MerchantCode"];
	if (![iPay88MerchantCode isKindOfClass:[NSNull class]]) {
		[GlobalPreferences setiPay88MerchantCode:iPay88MerchantCode];
	}

	//************************************Stripe Key**********************
	// 4/11/2014 Tuyen Stripe key
	NSString *stripePublishableKey = [dictUserSettings valueForKey:@"sStripePublishableKey"];
	if (![stripePublishableKey isKindOfClass:[NSNull class]])
		[GlobalPreferences setStripePublishableKey:stripePublishableKey];

	NSString *stripeSecretKey = [dictUserSettings valueForKey:@"sStripeSecretKey"];
	if (![stripeSecretKey isKindOfClass:[NSNull class]])
		[GlobalPreferences setStripeSecretKey:stripeSecretKey];

	NSString *parseApplicationId = [dictUserSettings valueForKey:@"sStripeApplicationId"];
	if (![parseApplicationId isKindOfClass:[NSNull class]])
		[GlobalPreferences setParseApplicationId:parseApplicationId];

	NSString *parseClientKey = [dictUserSettings valueForKey:@"sStripeClientKey"];
	if (![parseClientKey isKindOfClass:[NSNull class]])
		[GlobalPreferences setParseClientKey:parseClientKey];

	NSString *masterSecret = [dictUserSettings valueForKey:@"sStripeMasterKey"];
	if (![masterSecret isKindOfClass:[NSNull class]])
		[GlobalPreferences setMasterSecret:masterSecret];

	// Not have
	NSString *parseAppName = [dictUserSettings valueForKey:@"parseAppName"];
	if (![parseAppName isKindOfClass:[NSNull class]])
		[GlobalPreferences setParseAppName:parseAppName];

	NSString *isStripeModeLive = [dictUserSettings valueForKey:@"isStripeModeLive"];
	if (![isStripeModeLive isKindOfClass:[NSNull class]])
		[GlobalPreferences setStripeModeIsLive:isStripeModeLive];

	NSString *isStripeEnable = [dictUserSettings valueForKey:@"stripeUsed"];
	if (![isStripeEnable isKindOfClass:[NSNull class]])
		[GlobalPreferences setStripeModeEnable:isStripeEnable];

	BOOL isZeroDecimal = [dictUserSettings valueForKey:@"isZeroDecimal"];
	if (!isZeroDecimal)
		[GlobalPreferences setIsZeroDecimal:isZeroDecimal];

	BOOL isStripeLive = [dictUserSettings valueForKey:@"stripeLive"];
	if (!isStripeLive) {
		[GlobalPreferences setIsStripeLive:isStripeLive];
	}
	//4/11/2014 Tuyen End
    
    //***********************************Cash Key************************
    // 09/02/2015 Tuyen Cash fee key
    NSString *strDeliveryFee = [dictUserSettings valueForKey:@"fCodFee"];
    if (![strDeliveryFee isKindOfClass:[NSNull class]]) {
        float deliveryFee = [strDeliveryFee floatValue];
        [GlobalPreferences setCashDeliveryFee:deliveryFee];
    }
