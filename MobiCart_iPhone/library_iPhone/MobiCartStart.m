...
@synthesize objShopping = _objShopping;
// End
...
- (id)init {
	...
	//get delegate ref
	if (!delegate)
		delegate = (MobicartAppDelegate *)[[UIApplication sharedApplication]delegate];
...
{
    DDLogInfo(@"startMobicartOnMobicartView : STARTED");
    //DLog(@"3");
	self = [super init];
    ...
    DDLogInfo(@"startMobicartOnMobicartView : ENDED");
    return self;
}
...
- (void)createTabbarContorllers { //DLog(@"6");
	...
	for (int i = 0; i < [arrControllersToCreate count]; i++) {
		...
		if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
			localNavigationController.navigationBar.barTintColor = navBarColorPreference;
			...
			[localNavigationController.navigationBar.layer setBorderColor:[navBarColorPreference CGColor]];

			//Sa Vo fix bug not display status bar text on dark background on iOS 7.x
			if ([self isDarkColor:navBarColorPreference]) {
			...
			localNavigationController.navigationBar.tintColor = navBarColorPreference;
		}
	...
	//    [(MobicartAppDelegate*)[[UIApplication sharedApplication] delegate] ChangeMoreTab:(UINavigationController*)localControllersArray];
...
- (void)btnShoppingCart_Clicked:(id)sender {
	...
	if (selectedQuantity > 0 && isPostReviews == NO) { //[self performSelectorInBackground:@selector(showLoadingbar) withObject:nil];
	}

	[arrDatabaseCart release];

	// 01/12/2014 Tuyen close code to test bug 27702
	// 19/11/2014 Tuyen new code to test bug 27702
//	self.objShopping = [[[ShoppingCartViewController alloc] init] autorelease];
//
//	[[GlobalPreferences getCurrentNavigationController] pushViewController:self.objShopping animated:YES];
	// End
	//

	// 19/11/2014 Tuyen new code to test bug 27702
	if (_objShopping) {
		[_objShopping release];
		_objShopping = nil;
	}
	self.objShopping = [[ShoppingCartViewController alloc] init];

	[[GlobalPreferences getCurrentNavigationController] pushViewController:self.objShopping animated:YES];
	// End


	// 19/11/2014 Tuyen close code to test bug 27702
...
- (void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController1 {
	...
	//Sa Vo fix bug not display tint color of navigation bar on iOS 7.x
	if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7) {
		tabBarController.moreNavigationController.navigationBar.barTintColor = navBarColorPreference;
		...
		tabBarController.moreNavigationController.navigationBar.tintColor = navBarColorPreference;
	}
