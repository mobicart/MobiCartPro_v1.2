...
- (void)tableView:(UITableView *)tableview didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	// 21/11/2014 Tuyen close code to bug hidden cart after logout
//	if (rightContentView) {
//
//		NSArray *arrTemp = [rightContentView subviews];
//		if([arrTemp count] > 0)
...
//	}
	...
	switch (indexPath.row) {
		case 0:
		{
			// 21/11/2014 Tuyen new code to fix bug hidden cart after logout
			[self clearOldRightView];
			// End
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
	if (buttonIndex == 1) {
        ...
        // 13/01/2015 Tuyen new code fix bug 28382
        if (rightContentView) {
            NSArray *arrTemp = [rightContentView subviews];
            if ([arrTemp count] > 0) {
                for (int i = 0; i < [arrTemp count]; i++)
...
#pragma mark - Method
// 21/11/2014 Tuyen new code
- (void)clearOldRightView {
	if (rightContentView) {
		NSArray *arrTemp = [rightContentView subviews];
		if ([arrTemp count] > 0) {
			for (int i = 0; i < [arrTemp count]; i++)
