...
@synthesize objCheckout = _objCheckout;
// End
...
- (void)viewDidLoad {
    ...
    // 19/01/2015 Tuyen fix bug wrong tax, ship of order
    if (!dictSettingsDetails) {
...
- (void)btnEdit_clicked {
	...
	if ([btnEdit.titleLabel.text isEqualToString:[[GlobalPrefrences getLangaugeLabels]valueForKey:@"key.iphone.shopping.cart.edit.cart"]]) {
		...
		[pickerBar removeFromSuperview];
		[pickerViewQuantity removeFromSuperview];
		DDLogDebug(@"pickerViewQuantity->%d", [pickerViewQuantity retainCount]);
...
//	DDLogDebug(@"hight %f",tableView.frame.size.height+tableView.frame.origin.x+10);
	if (viewFooter) {
...
//			[objCheckout release];
			...
			if (_objCheckout != nil) {
				[_objCheckout release];
				_objCheckout = nil;
			}
			self.objCheckout = [[CheckoutViewController alloc]init];
			// 11/11/2014 Tuyen new fix bug wrong GUI when present view iOS8 - Stripe and iPay88
			self.objCheckout.superVC = self;
			// End
			NSMutableArray *arrTemp = [[NSMutableArray alloc] init];
			...
			self.objCheckout.arrProductIds = arrTemp;
			self.objCheckout.arrCartItems = arrDatabaseCart;

			[arrTemp release];
			[rightContentView addSubview:self.objCheckout.view];
			// End
		}
		else {
...
- (UITableViewCell *)tableView:(UITableView *)tableview cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (tableview == tblCountries) {
		...
		if (cell == nil) {
			...
			// 12/01/2014 Tuyen close code fix bug edit quantity of item wrong
//			lblQuantity.tag = [[NSString stringWithFormat:@"99%d0%d", indexPath.row + 1, indexPath.row + 1] intValue];
			//

			// 12/01/2014 Tuyen close code fix bug edit quantity of item wrong
			lblQuantity.tag = [[NSString stringWithFormat:@"99%d", indexPath.row + 100] intValue];
			//
...
//			btnQuantity.tag = [[NSString stringWithFormat:@"%d0%d", indexPath.row + 1, indexPath.row + 1] intValue];
			//
			// 12/01/2014 Tuyen new code fix bug edit quantity of item wrong
			btnQuantity.tag = [[NSString stringWithFormat:@"%d", indexPath.row + 100] intValue];
			//
...
//	UIButton *btnQuantity_Temp = (UIButton *)[cell viewWithTag:[[NSString stringWithFormat:@"%d0%d", indexPath.row + 1, indexPath.row + 1] intValue]];
	...
	// 12/01/2014 Tuyen new code fix bug edit quantity of item wrong
	UIButton *btnQuantity_Temp = (UIButton *)[cell viewWithTag:[[NSString stringWithFormat:@"%d", indexPath.row + 100] intValue]];
	//
...
//	UIButton *btnQuantity_Temp = (UIButton *)[tblViewCell viewWithTag:[[NSString stringWithFormat:@"%d0%d", indexPath.row + 1, indexPath.row + 1] intValue]];
	...
	// 12/01/2014 Tuyen new code fix bug edit quantity of item wrong
	UIButton *btnQuantity_Temp = (UIButton *)[tblViewCell viewWithTag:[[NSString stringWithFormat:@"%d", indexPath.row + 100] intValue]];
	//
...
//		int selectedRow = [[temp objectAtIndex:0] intValue] - 1;
		...
		// 12/01/2014 Tuyen new code fix bug edit quantity of item wrong
		int selectedRow = iTagOfCurrentQuantityBtn - 100;
		//
...
//	[[SqlQuery shared] updateTblShoppingCart:[[btnTemp titleForState:UIControlStateNormal] intValue]:[[[arrDatabaseCart objectAtIndex:(iTagOfCurrentQuantityBtn % 10) - 1] valueForKey:@"id"] intValue]:[[arrDatabaseCart objectAtIndex:(iTagOfCurrentQuantityBtn % 10) - 1] valueForKey:@"pOptionId"]];
	...
	// 12/01/2014 Tuyen new code fix bug edit quantity of item wrong
	[[SqlQuery shared] updateTblShoppingCart:[[btnTemp titleForState:UIControlStateNormal] intValue]:[[[arrDatabaseCart objectAtIndex:iTagOfCurrentQuantityBtn - 100] valueForKey:@"id"] intValue]:[[arrDatabaseCart objectAtIndex:iTagOfCurrentQuantityBtn - 100] valueForKey:@"pOptionId"]];
	//
