...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
	...
	if (self) {
		...
		// 12/01/2015 Tuyen close fix bug crash
//        if (!arrSearch)
//        {
...
- (void)viewDidLoad {
	...
	// 12/01/2015 Tuyen test fix bug leak memory
...
- (void)fetchDataFromServer {
	...
	// DDLogInfo(@"fetch data from server home controller");
	[pool release];
}
	...
	if (![arrTemp isKindOfClass:[NSNull class]]) {
		if ([arrTemp count] > 0) {
			...
			if (!isTryingSecondTime) {
				DDLogWarn(@"No Data Available for this Store (StoreViewContoller)  --> TRYING AGAIN TO FETCH DATA ");
				isTryingSecondTime = TRUE;
				[self fetchDataForDepartments];
			}
			else
				DDLogWarn(@"No Data Available for this Store (StoreViewContoller)");
		}
	}

	else {
		DDLogWarn(@"No Data Returned from server (StoreViewContoller)");
	}
...
- (void)createDynamicControls {
	if (dictFeaturedProducts) {
		...
		if ((![arrAllData isEqual:[NSNull null]]) && (arrAllData != nil)) {
			...
			DDLogWarn(@"No Featured Products available (Home View Controller");
	}
	else
		DDLogWarn(@"No Featured Products available (Home View Controller");
}
...
- (void)sendDataToServer2:(NSDictionary *)dictShoppingCartQueueData {
	...
	if ([reponseRecieved isKindOfClass:[NSString class]]) {
		...
		for (int i = 0; i < [arrIndividualProducts count]; i++) {
			...
			if (iCurrentOrderId > 0) {
				...
				DDLogInfo(@"INTERNET IS UNAVAILABLE, KEEPING DATA IN THE LOCAL DATABASE");
