...
- (void)decodeDescription:(NSDictionary *)dictionary {
	...
	if (tag == WEBVIEWFIRST_TAG) {
		...
		htmlString = [@"test product" kv_decodeHTMLCharacterEntities]; //[string kv_decodeHTMLCharacterEntities];
...
//					DDLogDebug(@"%d", minQuantityCheck[count]);
					}
...
//						DDLogDebug(@"%d",optionSizeIndex[i]);
					}
				}
...
//								DDLogDebug(@"%d",quantityAdded[i]);
				...
				if ([wishlistOptions count] > 0) {
					DDLogDebug(@"%d", optionSizeIndex[0]);
