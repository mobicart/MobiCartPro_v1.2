...
-(void)sortSegementChanged:(id)sender
{
	...
	@catch (NSException * e)
	{
		DDLogError(@"Error While Sorting (ProductViewController)");
	}
	@finally
	{
		[arrAllData retain];
...
{
    // 12/01/2014  Tuyen close fix bug crash
//	arrAllData=[[NSArray alloc]init];
    //
    
    // 12/01/2014  Tuyen new code fix bug crash
    if (!arrAllData) {
        arrAllData = [[NSArray alloc] init];
    }
    if (!arrSearch) {
...
//	DDLogDebug(@"Button tag is %d",sender.tag-100);
	[NSThread detachNewThreadSelector:@selector(showLoading) toTarget:self withObject:nil];
...
- (BOOL)popoverControllerShouldDismissPopover:(UIPopoverController *)popoverController {
    ...
    DDLogInfo(@"popover about to be dismissed");
    return YES;
}
...
- (void)dealloc {
    ...
    // 12/01/2015 Tuyen new code bug crash
    if (arrAllData) {
        [arrAllData release];
    }
    if (arrSearch) {
