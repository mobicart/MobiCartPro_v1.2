...
-(NSMutableArray *) getAccountData:(NSString *)eMailAddress
{
	...
	sql4 = [str UTF8String];
	NSMutableArray *arrAccountData=[[NSMutableArray alloc]init];	
...
-(NSMutableArray *) getShoppingProductSizes:(int)productID
{
	...
	NSMutableArray *arrProductOption=[[NSMutableArray alloc]init];
...
- (NSMutableArray *)getShoppingCartQueue
{
	...
	NSMutableArray *arrShoppingCartQueue = [[NSMutableArray alloc] init];
...
{
	NSString *strTimeStamp = [[[[[NSString stringWithFormat:@"%@",[dictData valueForKey:@"timestamp"]] stringByReplacingOccurrencesOfString:@"(" withString:@""] stringByReplacingOccurrencesOfString:@")" withString:@""] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] stringByReplacingOccurrencesOfString:@"\"" withString:@""] ;
	
	sqlite3_stmt *insert_statement = nil;
	static char *sql=nil;
	sql= "insert into TblLanguageLabels(sKey,sValue) values(?,?)";
	
	NSArray *arrKeys = [NSArray arrayWithArray:[[[dictData valueForKey:@"labelsMap"] objectAtIndex:0]allKeys] ];
	...
	for (int i=0; i<[arrKeys count]; i++) 
	{
		...
		sqlite3_bind_text(insert_statement, 2, [[[[dictData valueForKey:@"labelsMap"] objectAtIndex:0] valueForKey:[arrKeys objectAtIndex:i]] UTF8String] , -1, SQLITE_TRANSIENT);
		
		if(SQLITE_DONE != sqlite3_step(insert_statement))
		{
			NSLog(@"Error while inserting Data. %@", sqlite3_errmsg(database));
		}
