...
- (void) setUpLayers {
	...
	topPageShadow.colors = [NSArray arrayWithObjects:
							(id)[[backGroundColor colorWithAlphaComponent:0.6] CGColor],
	...
	[bottomPage setContents:contentView];
	...
	basePage = [[CALayer alloc] init];
	basePage.masksToBounds = YES;
	basePage.contentsGravity = kCAGravityLeft;
	basePage.frame=CGRectMake(0, 0, 1024, 768);
	
	StoreViewController *obj=[[StoreViewController alloc]init];
	...
	[topPage addSublayer:basePage];
	[topPage addSublayer:obj.btnProductImage.layer];
...
- (void)setUpLayersForViewingMode {
    if (self.mode == LeavesViewModeSinglePage) {
        ...
        topPageReverseOverlay.backgroundColor = [[backGroundColor colorWithAlphaComponent:0.8] CGColor];
        topPageReverseImage.transform = CATransform3DMakeScale(1, 1, 1);
    }
}
...
- (void) setLayerFrames {
	...
	topPage.frame = CGRectMake(rightPageBoundsRect.origin.x-520, 
							   rightPageBoundsRect.origin.y, 
							   leafEdge * rightPageBoundsRect.size.width*2, 
...
- (void) didTurnPageBackward {
	...
	StoreViewController *obj=[[StoreViewController alloc]init];
	obj.firstPageIndex=startIndex-18;
	obj.isBack=YES;
	[obj createBasicControls];
...
- (void) didTurnPageForward {
	...
	obj.firstPageIndex=currentPageIndex-1;
	[obj createBasicControls];
	[self didTurnToPageAtIndex:currentPageIndex];
}
